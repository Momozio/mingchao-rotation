...
const toggleEnvironment = (env) => {
  ...
  if (idx > -1) {
    ...
    newTeam.value.environment.push(env)
  }
}
...
const openAddModal = () => {
  ...
  newTeam.value.characters = [
    { id: null, name: null, star: null, weapon: null, element: null, energy: '无' },
    { id: null, name: null, star: null, weapon: null, element: null, energy: '无' },
    { id: null, name: null, star: null, weapon: null, element: null, energy: '无' }
  ]
  ...
  <div class="h-screen flex flex-col bg-[var(--bg-primary)] overflow-hidden">
    <header class="flex-shrink-0 bg-[var(--bg-secondary)] shadow-sm border-b border-[var(--border-color)]">
      <div class="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <button 
          @click="openAddModal"
          class="px-4 py-2 rounded-lg bg-[var(--accent-color)] text-white text-sm font-medium hover:opacity-90 transition-opacity flex items-center gap-2"
        >
    ...
    <CharacterFilter ref="filterRef" @team-change="handleTeamChange">
      <div class="max-w-3xl mx-auto py-6">
        <!-- 已有配队列表 -->
        <div v-if="teams.length > 0" class="px-4 mb-8 space-y-4">
          <div 
            v-for="t in teams" 
            :key="t.id"
            class="bg-[var(--bg-secondary)] rounded-2xl shadow-sm border border-[var(--border-color)] p-5"
          >
            <div class="flex items-start justify-between mb-4">
              <div>
                <h3 class="text-lg font-semibold text-[var(--text-primary)]">{{ t.name }}</h3>
                <p class="text-xs text-[var(--text-tertiary)] mt-0.5">{{ t.remark }}</p>
              </div>
              <span class="text-xs text-[var(--text-tertiary)]">#{{ t.id }}</span>
            </div>
              ...
              <div v-for="(char, idx) in t.characters" :key="idx" class="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-1">
                    ...
                    <span v-if="char.energy !== '无'" class="text-[10px] text-cyan-400">{{ char.energy }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 mb-4">
              <div class="bg-[var(--bg-tertiary)] rounded-lg p-3 text-center">
                <div class="text-lg font-semibold text-[var(--text-primary)]">{{ t.axisLength || '-' }}s</div>
                <div class="text-xs text-[var(--text-tertiary)]">轴长</div>
              </div>
              <div class="bg-[var(--bg-tertiary)] rounded-lg p-3 text-center">
                <div class="text-lg font-semibold text-[var(--text-primary)]">{{ t.dps || '-' }}w</div>
                <div class="text-xs text-[var(--text-tertiary)]">DPS</div>
              </div>
              <div class="bg-[var(--bg-tertiary)] rounded-lg p-3 text-center">
                <div class="text-lg font-semibold text-[var(--text-primary)]">{{ t.difficulty }}</div>
                <div class="text-xs text-[var(--text-tertiary)]">难度</div>
              </div>
            </div>

            <div class="border-t border-[var(--border-color)] pt-4">
              <div v-if="t.flow.startup || t.flow.loop" class="text-xs font-medium text-[var(--text-secondary)] mb-2">输出流程</div>
        ...
        <div v-else class="text-center py-12 px-4">
          <div class="text-[var(--text-tertiary)]">暂无配队</div>
          <div class="text-[var(--text-tertiary)] text-sm mt-2">点击右上角添加配队</div>
        </div>
      </div>
    </CharacterFilter>
    ...
    <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-2xl bg-[var(--bg-secondary)] rounded-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        <div class="flex items-center justify-between p-4 border-b border-[var(--border-color)]">
          <h2 class="text-lg font-semibold text-[var(--text-primary)]">添加配队</h2>
          <button @click="showAddModal = false" class="p-1.5 rounded-lg hover:bg-[var(--bg-tertiary)]">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-5">
          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-1.5">配队名称 <span class="text-red-500">*</span></label>
            <input 
              v-model="newTeam.name"
              type="text" 
              placeholder="输入配队名称"
              class="w-full px-3 py-2.5 bg-[var(--bg-tertiary)] rounded-lg text-sm text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)]/30"
            >
          </div>

          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-1.5">备注（可选）</label>
            <input 
          ...
          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-2">选择角色 <span class="text-red-500">*</span></label>
            <div class="flex justify-center gap-4 mb-3">
              ...
              >
                ...
                >
                  <div class="aspect-square rounded-xl bg-[var(--bg-tertiary)] border-2 border-[var(--accent-color)] flex flex-col items-center justify-center p-1 overflow-hidden">
                    <img 
                      :src="`/assets/characters/${char.name}.webp`" 
                      :alt="char.name"
                ...
                <div
                  ...
                  class="aspect-square rounded-xl border-2 border-dashed border-[var(--border-color)] flex items-center justify-center cursor-pointer hover:border-[var(--accent-color)] hover:bg-[var(--bg-tertiary)] transition-colors"
                >
                  <span class="text-xs text-[var(--text-tertiary)]">+ 选择</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-1.5">适配环境</label>
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              <label class="block text-sm text-[var(--text-secondary)] mb-1.5">轴长(s)</label>
              <input 
                v-model="newTeam.axisLength"
                type="text" 
                placeholder="20"
                class="w-full px-3 py-2 bg-[var(--bg-tertiary)] rounded-lg text-sm text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)]/30 text-center"
              >
            </div>
            <div>
              <label class="block text-sm text-[var(--text-secondary)] mb-1.5">DPS(w) / 矩阵分数</label>
              <div class="grid grid-cols-2 gap-2">
                <div>
                  <input 
                    ...
                    type="text" 
                    placeholder="DPS"
                    class="w-full px-2 py-2 bg-[var(--bg-tertiary)] rounded-lg text-xs text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)]/30 text-center"
                  >
                </div>
                <div>
                  <input 
          ...
          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-1.5">启动轴</label>
            <textarea 
              v-model="newTeam.flow.startup"
              placeholder="角色A开E→角色B QTE→角色C E..."
              rows="2"
              class="w-full px-3 py-2.5 bg-[var(--bg-tertiary)] rounded-lg text-sm text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)]/30 resize-none"
            ></textarea>
          </div>

          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-1.5">循环轴</label>
            <textarea 
          ...
          <div>
            <label class="block text-sm text-[var(--text-secondary)] mb-1.5">贡献者</label>
            <input 
        ...
        <div class="p-4 border-t border-[var(--border-color)] flex gap-3">
          <button 
            @click="showAddModal = false"
            class="flex-1 py-2.5 rounded-xl bg-[var(--bg-tertiary)] text-[var(--text-primary)] font-medium text-sm hover:opacity-80 transition-opacity"
          >
            取消
          </button>
          <button 
            @click="saveTeam"
            class="flex-1 py-2.5 rounded-xl bg-[var(--accent-color)] text-white font-medium text-sm hover:opacity-90 transition-opacity"
          >
            保存
          </button>
        </div>
      ...
      <div v-if="showCharacterPicker" class="fixed inset-0 z-60 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/60" @click="showCharacterPicker = false"></div>
        <div class="relative w-full max-w-2xl bg-[var(--bg-secondary)] rounded-2xl shadow-xl overflow-hidden max-h-[80vh] flex flex-col">
          <div class="flex items-center justify-between p-4 border-b border-[var(--border-color)]">
            <h2 class="text-lg font-semibold text-[var(--text-primary)]">选择角色</h2>
            <button @click="showCharacterPicker = false" class="p-1.5 rounded-lg hover:bg-[var(--bg-tertiary)]">
              <svg class="w-5 h-5 text-[var(--text-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <div class="flex-1 overflow-y-auto p-4">
            <div class="grid grid-cols-5 gap-2">
              <div
                ...
                class="rounded-lg bg-[var(--bg-tertiary)] hover:bg-[var(--accent-color)]/20 border border-transparent hover:border-[var(--accent-color)] cursor-pointer transition-all flex flex-col items-center justify-center p-2 overflow-hidden"
              >
                <img 
                  :src="`/assets/characters/${char.name}.webp`" 
                  :alt="char.name"
